...
def build_cnn(seq_length):
  ...
  model = tf.keras.Sequential([
      ...
      tf.keras.layers.Dense(2, activation="softmax")  # (batch, ?) class_count
  ])
...
def train_net(
  ...
  open("kaiTan.tflite", "wb").write(tflite_model)
  ...
  # Save the model to disk
  open("kaiTan_quantized.tflite", "wb").write(tflite_model)
  ...
  basic_model_size = os.path.getsize("kaiTan.tflite")
  print("Basic model is %d bytes" % basic_model_size)
  quantized_model_size = os.path.getsize("kaiTan_quantized.tflite")
